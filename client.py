...
def parse_arguments():
    ...
    parser.add_argument(
        "--desc", type=str, required=True, help="User input describing preference"
    )
    parser.add_argument(
        "--topn", type=int, default=5, help="Number of top recommendations to return"
    )
    parser.add_argument(
        "--model",
        type=str,
        default="tfidf",
        choices=["tfidf", "tfidf-lemmatized", "tfidf-svd", "sbert"],
        help="Model type for recommendations",
    )
    parser.add_argument(
...
def main():
    ...
    topn = (
        max(topn, 10) if topn >= 10 else topn
    )  # ensure that no more than 10 recommendations are made
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for idx, (title, overview, score) in enumerate(
            zip(
            ...
            print(
                f"{idx+1}. {title} (Similarity: {score:.4f})\n📜 Overview: {overview}\n",
                file=f,
            )
